...
async def create_invitation_by_owner(
    ...
    db_invitation = Invitation(
        ...
        clubs_limit=0,  # Не-owner не могут создавать клубы
        club_id=club_id,
...
async def update_invitation(
    ...
    # Только создатель или суперадмин может обновлять
    updater = await session.get(UserStaff, updater_id)
    if not updater.is_superadmin and invitation.created_by_id != updater_id:
        raise PermissionError("You can only update invitations you created")

    # Нельзя обновлять использованные приглашения
    if invitation.is_used:
        raise ValueError("Cannot update used invitation")
    ...
    await session.refresh(invitation, ["club", "created_by"])
