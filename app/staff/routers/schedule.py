...
# ===== LESSON PARTICIPANTS ENDPOINTS =====


@router.get("/lessons/{lesson_id}/participants")
@limiter.limit("30/minute")
async def get_lesson_participants_for_staff(
    request: Request,
    lesson_id: int = Path(..., description="Lesson ID"),
    db: AsyncSession = Depends(get_session),
):
    """
    Get list of participants for a specific lesson.
    
    Returns both booked and excused (frozen) students.
    Staff can see excuse notes.
    """
    from app.students.crud.bookings import get_lesson_participants
    ...
    booked, excused, booked_count, excused_count, max_participants = await get_lesson_participants(
        db,
        lesson_id=lesson_id,
        current_student_id=0,  # No current student for staff view
    )
    
    return {
        "lesson_id": lesson_id,
        "participants": booked,
        "excused_participants": excused,
        "total": booked_count,
