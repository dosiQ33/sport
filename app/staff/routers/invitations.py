...
@router.put("/{invitation_id}", response_model=InvitationRead)
@limiter.limit("10/minute")
async def update_invitation_route(
    request: Request,
    invitation_id: int,
    ...
    """
    Обновить приглашение (только создатель).
    Нельзя обновить использованное приглашение.
    """
    user_staff = await get_user_staff_by_telegram_id(db, current_user.get("id"))
    if not user_staff:
        raise HTTPException(status_code=404, detail="Staff user not found")
    ...
    try:
        updated_invitation = await update_invitation(
            db, invitation_id, invitation_update, user_staff.id
        )
