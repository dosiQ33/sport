...
from app.stuff.schemas.clubs import ClubCreate, ClubUpdate, ClubRead, ClubListResponse
from app.stuff.crud.users import get_user_stuff_by_telegram_id
from app.stuff.crud.clubs import (
    get_club_by_id,
    get_clubs_by_owner,
    get_clubs_paginated,
    create_club,
...
async def create_new_club(
    ...
    try:
        db_club = await create_club(db, club, user_stuff.id)
        return db_club
...
async def get_my_clubs(
    ...
    clubs = await get_clubs_by_owner(db, user_stuff.id)
    return clubs
...
async def update_club_details(
    ...
    has_permission = await check_user_club_permission(db, user_stuff.id, club_id)
    ...
    try:
        db_club = await update_club(db, club_id, club_update, user_stuff.id)
        if not db_club:
            raise HTTPException(status_code=404, detail="Club not found")
        return db_club
    except ValueError as e:
...
async def delete_club_route(
    ...
    ⚠️ **Warning**: This action is irreversible and will also delete all related sections and user roles.
    """
    user_stuff = await get_user_stuff_by_telegram_id(db, current_user.get("id"))
    if not user_stuff:
        raise HTTPException(status_code=404, detail="Staff user not found")
    ...
    try:
        deleted = await delete_club(db, club_id, user_stuff.id)
        if not deleted:
            raise HTTPException(status_code=404, detail="Club not found")
    except PermissionError as e:
        raise HTTPException(status_code=403, detail=str(e))
...
async def check_club_permission(
    ...
    has_permission = await check_user_club_permission(db, user_stuff.id, club_id)
    is_owner = club.owner_id == user_stuff.id
